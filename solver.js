export class WordleSolver {
    ...
    findBestWord() {
        ...
        if (candidateWords.length > 2) {
            let charFrequency = new Map(); //Key = character, Value = Number of Times it is used in candidateWords
            const maxDiff = this.highestCharDifference(candidateWords);
            if (maxDiff == 1 || maxDiff == 2) {
                ...
                for (let i = 0; i < 5; i++) {
                    if (this.hasDifferenCharAt(i, candidateWords)) {
                        for (const s of candidateWords) {
                            const c = s.charAt(i);
                            const currentCount = (charFrequency.get(c) || 0) + 1;
                            ; //If Element isnt in charFrequency yet it should be inserted with count=1
                ...
                for (const s of this.words) { //search for word that eliminates most words of candidates
                    let currentScore = 0;
                    let uniqueLettersWord = new Set(s.split(""));
    ...
    highestCharDifference(strings) {
        ...
        for (let i = 0; i < strings.length; i++) {
            for (let j = i + 1; j < strings.length; j++) {
                ...
                if (diff == 5) {
                    return 5;
                } //Since all Words are Length 5 the max Difference of Chars can be 5 so we can return it here
                if (diff > biggestDiff) {
                    biggestDiff = diff;
                }
            }
    ...
    hasDifferenCharAt(index, strings) {
        if (strings.length == 0) {
            return false;
        }
    ...
    matchesGreenPatern(s) {
        if (this.greenLetters.size == 0) {
            return true;
        }
        for (let i = 0; i < 5; i++) {
    ...
    matchesOrangePatterns(s) {
        ...
        const allOrangeLetters = Array.from(new Set([
            ...this.position0List,
            ...this.position1List,
            ...this.position2List,
            ...this.position3List,
